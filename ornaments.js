(function () {
  ...
  function drawMainStar(canvas, size) {
    if (!canvas) return;
    const W = size || 340, H = size || 340;
  ...
  function drawAllStars() {
    // Matches the legacy single #star-canvas plus any .star-canvas elements.
    // Size comes from the width attribute (read via getAttribute so it stays
    // stable after dpr() rescales the .width property), default 340.
    const stars = document.querySelectorAll('#star-canvas, .star-canvas');
    stars.forEach(function (c) {
      drawMainStar(c, parseInt(c.getAttribute('width'), 10) || 340);
    });
  }
